...
- **Double-click = audible COMPARE** (PLAY/EDIT/PERFORMANCE): the first edit after a program change snapshots the voice; `COPMEmu::setCompare(true)` makes `getPatch()` serve the snapshot. Refused (status "NO EDITS YET") when nothing was edited. Program change resets the compare/dirty context and syncs `m_sysexEditVoice` to the program (RAM 0-31; ROM programs are read-only until loaded via the MEMORY ROM actions).
- **MEMORY mode has 7 actions** (stage machine 0=action, 1=Y/N, 2=picker 1, 4=picker 2, 3=result): Save/Load/Verify SD bank (incl. `performances.json`), Store Voice (slot 1-32), ROM Grp→Bank (A11), ROM Voice→Slot (A12), Store Perf (slot 1-32).
- **PERFORMANCE mode** drives the 32 performance memories: value-rotation steps `m_PerfSlot` and calls `ApplyPerformance()`; the renderer shows name, play-mode tag (SI/DU/SP) and voice A/B. Store via the MEMORY "PERF" action.
- **FUNCTION "Name :" (#45)** opens the name editor: rotation cycles the character, click advances, click on position 10 commits via `SetVoiceName()` (Memory Protect enforced). While active, the input layer routes FUNCTION events to the editor.
- **Edit transmit**: `writeVcedGlobal/Operator(..., transmit)` send the real-time parameter change (`F0 43 1n 12 pp vv F7`, real VCED numbering) when "Midi Sy Info" is ON; `applySysexParam` passes `transmit=false` so incoming MIDI never echoes. Panel voice select additionally sends a 1-voice dump (`transmitVoiceIfEnabled`).
- **SysEx out**: the kernel's `ForwardMIDI` is registered as SysEx-out callback on adapter AND engine. The engine answers Yamaha dump requests (`F0 43 2n ff F7`, f=3 → 1-voice VCED, f=4 → 32-voice VMEM bulk; gated on FUNCTION #5 "Midi Sy Info"); FUNCTION #41 "Midi Transmit ?" triggers the bulk via `TriggerBulkTransmit()`.
- **SysEx formats are hardware-DX21-compatible** (manual tables 5-1/5-2): 1-voice VCED = format 0x03, 93 bytes; 32-voice bulk = format 0x04, 4096 bytes (73-byte packed VMEM + 55 pad per voice); checksum = two's complement of the data sum; real-time parameter change uses VCED numbering 0–92 plus function param 93 (operator enable). Wire op-block order is OP4, OP2, OP3, OP1. The pre-compat project formats (0x09 bulk / 76-byte VCED, simple-sum checksum) are still accepted on RECEIVE only. Memory Protect is enforced on all imports; edit slot = `m_sysexEditVoice`.
