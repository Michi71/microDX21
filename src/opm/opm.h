...
typedef struct {
    ...
    // OPP attack-rate slowdown (from nuked-opp-xcent):
    // Per-slot skip counter to model YM2164 AR=18-30 plateau.
    // skip=N zeros every Nth attack inc=1 tick, making attack (N+1)/N times longer.
    // skip=0 disables (default, = standard OPM behavior).
...
void OPM_SetAttackSkip(opm_t *chip, uint8_t slot, uint8_t skip);
