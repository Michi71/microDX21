...
// main loop (MIDI, LVGL, deferred preset load).
//
...
LOGMODULE("velvetkeys");

CVelvetKeys::CVelvetKeys(CConfig*          pConfig,
...
, m_audioPaused(false)
{
    ...
    // Real value is loaded from velvetkeys.ini in Initialize().
...
bool CVelvetKeys::Initialize()
{
    ...
    m_piano.setSysexOutCallback(
          ...
          {
            CVelvetKeys* self = static_cast<CVelvetKeys*>(user);
            if (!self || !data || len == 0)
                return;
    ...
    if (m_pConfig->IsUSBGadget())
    {
        ...
        m_usbMidi[0] = new CVelvetUSBMIDIDevice(this, m_pConfig, 0);
    ...
    else
    {
        ...
        for (unsigned i = 0; i < MaxUSBMIDIDevices; i++)
            m_usbMidi[i] = new CVelvetUSBMIDIDevice(this, m_pConfig, i);
    }

    m_serialMidi = new CVelvetSerialMIDIDevice(this, m_pInterrupt, m_pConfig);
    ...
    m_pcKeyboard = new CVelvetPCKeyboard(this, m_pConfig);

    return true;
}

void CVelvetKeys::Process(bool plugAndPlayUpdated)
{
    for (unsigned i = 0; i < MaxUSBMIDIDevices; i++)
        if (m_usbMidi[i])
...
unsigned CVelvetKeys::GenerateAudio(float* outL, float* outR, unsigned nFrames)
{
    if (nFrames > 4096)
        nFrames = 4096;
...
bool CVelvetKeys::FillAudioSlot()
{
...
bool CVelvetKeys::GetAudioChunk(float* outL, float* outR, unsigned& outFrames)
{
...
void CVelvetKeys::ProcessDeferredWork()
{
...
void CVelvetKeys::NoteOn(int32_t note, int32_t velocity)
{
    EnterCritical();
...
void CVelvetKeys::NoteOff(int32_t note)
{
    EnterCritical();
...
void CVelvetKeys::SendMidiCmd(uint8_t cmd, uint8_t data1, uint8_t data2)
{
    EnterCritical();
...
// Channel filter — consulted by CVelvetMIDIDevice::HandleMIDI before
// dispatching channel-voice messages to the synth engine.  See header.
bool CVelvetKeys::ShouldAcceptChannel(u8 channel0Based) const
{
    if (m_MidiChannel <= 0)
        return true;                                    // Omni
...
void CVelvetKeys::ForwardMIDI(const u8* msg, size_t len, CVelvetMIDIDevice* source)
{
    for (unsigned i = 0; i < MaxUSBMIDIDevices; i++)
    {
...
void CVelvetKeys::HandleSysExFromDevice(const u8* msg, size_t len, CVelvetMIDIDevice* source)
{
    if (!msg || len < 2)
        return;
...
void CVelvetKeys::ProcessDeferredSysEx()
{
    while (true)
    {
...
void CVelvetKeys::SetParameter(int index, float value)
{
    EnterCritical();
...
void CVelvetKeys::SetInstrument(int id)
{
    if (id < 0) return;
    ...
    if (id >= numPresets) {
        CLogger::Get()->Write("velvetkeys", LogError, "SetInstrument: index %d out of range (count=%d)", id, numPresets);
        return;
    }
...
int CVelvetKeys::GetInstrument()
{
    return m_piano.getInstrument();
}

void CVelvetKeys::GetInstrumentName(char* name)
{
    if (!name) return;
...
std::string CVelvetKeys::GetInstrumentNameByID(int id)
{
...
int CVelvetKeys::GetInstrumentCount()
{
...
int CVelvetKeys::GetNumPresets()
{
    return m_piano.getNumPresets();
}

const PresetInfo* CVelvetKeys::GetPreset(int index)
{
    return m_piano.getPreset(index);
}

void CVelvetKeys::LoadPreset(int index)
{
    if (index < 0) return;
    ...
    int numPresets = m_piano.getNumPresets();
    if (index >= numPresets) {
        CLogger::Get()->Write("velvetkeys", LogError, "LoadPreset: index %d out of range (count=%d)", index, numPresets);
        return;
    }
...
int CVelvetKeys::SaveUserPreset(const std::string& name,
...
void CVelvetKeys::SetMasterVolume(float volNorm)
{
...
int CVelvetKeys::GetMasterVolume127() const
{
...
int CVelvetKeys::getMidiSetting(int id) const
{
    switch (id)
    {
...
void CVelvetKeys::setMidiSetting(int id, int value)
{
    switch (id)
    {
        case kMidiChannel:
...
int CVelvetKeys::applyVelocityCurve(int vel) const
{
...
int CVelvetKeys::applyTranspose(int note) const
{
    int n = note + m_Transpose;
    return std::clamp(n, 0, 127);
}

const char* CVelvetKeys::GetAudioBackendName() const
{
    return m_pConfig->GetSoundDevice();
}

const char* CVelvetKeys::GetAudioDeviceName() const
{
    return "";
}

unsigned CVelvetKeys::GetSampleRate() const
{
    return m_sampleRate;
}

unsigned CVelvetKeys::GetPolyphony() const
{
    return m_maxPolyphony;
}

unsigned CVelvetKeys::GetChunkSize() const
{
    return m_pConfig->GetChunkSize();
}

bool CVelvetKeys::IsStereoSwapped() const
{
    return m_bChannelsSwapped;
}

bool CVelvetKeys::IsUSBDeviceMode() const
{
    return m_pConfig && m_pConfig->IsUSBGadget();
}

unsigned CVelvetKeys::GetUSBGadgetPin() const
{
    return m_pConfig ? m_pConfig->GetUSBGadgetPin() : 0;
}

const char* CVelvetKeys::GetFirmwareVersion() const
{
    return "1.0.0";
}